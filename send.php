...
$contact = $_POST['contact_num'];
        ...
        $mail->Body= 'Good day Mr/Ms.'.$last_name.'<br>'.'Your request to use the facility '.$facility_name.' for the date '.date('m/d/Y', strtotime($event_date)).' and time '.date('h:i A', strtotime($start)).' - '.date('h:i A', strtotime($end)).' has been '.$remarks.'.';
        if (!$mail->send()) {
